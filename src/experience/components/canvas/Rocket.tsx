...
const NOZZLE_OFFSETS = ROCKET_EXHAUSTS.map((e) => new THREE.Vector3(...e.pos));
...
export default function Rocket() {
  ...
  const geo = useMemo(() => {
    ...
    const plume = createRocketPlumeGeometry();
  ...
  const mats = useMemo(() => {
    ...
    const plumeOuter = createRocketPlumeMaterial(
    ...
    const plumeInner = createRocketPlumeMaterial(
  ...
  return (
    <>
      <group ref={rootRef}>
        ...
        {ROCKET_EXHAUSTS.map((e, i) => (
          <group key={`exhaust-${i}`} position={e.pos} scale={e.scale}>
            <mesh
              ref={(m) => {
